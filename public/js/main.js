...
console.log("%cPassword: '" + password + "' - Try it", 'color: grey');
...
function enterKey(e) {
  ...
  if (pw) {
    ...
    if (e.keyCode == 13) {
      ...
      git = commands.length;
      addLine('guest@ABTerminal:~$ ' + command.innerHTML, 'no-animation', 0);
...
function commander(cmd) {
  switch (cmd.toLowerCase()) {
    ...
    case 'sudo':
      ...
      break;
    case 'password':
      addLine('<span class="inherit"> Nice try)</span>', 'error', 100);
      break;
    case 'history':
      addLine('<br>', '', 0);
